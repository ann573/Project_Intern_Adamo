...
const DetailTour = () => {
  ...
  const totalRating = tour && (tour?.rating?.reduce((acc, cur) => acc + cur.rate, 0) / tour?.rating?.length).toFixed(2)
  ...
  useEffect(() => {
    if (tour) dispatch(getTours({ page: 1, limit: 6, request: `type_like=${tour?.type}` }))
  }, [tour, dispatch])
  ...
  return (
    <main className='max-w-[1200px] mx-auto xl:px-0 px-5 mb-20'>
      ...
      <h1 className='md:text-4xl text-3xl font-semibold md:w-2/3'>{tour?.title}</h1>
      ...
      <section className='grid lg:grid-cols-3 grid-cols-2 xl:gap-15 gap-5'>
        <div className='col-span-2 order-2 lg:order-1'>
          ...
          <section className='w-full'>
            <h3 className='font-bold my-5 text-2xl text-heading'>{t('related')}</h3>
            <div className='grid grid-cols-1 md:grid-cols-2 3xl:xl:grid-cols-3 md:gap-5 gap-y-10 '>
              {tours && tours?.map((item) => <Card data={item} key={item.id} />)}
            </div>
          </section>
        </div>
        ...
        <div className='order-1 lg:order-2 md:col-span-1 col-span-2'>
          <form className='bg-secondary dark:bg-secondary  sticky top-5'>
            <h2 className='py-7 px-5 font-medium text-xl'>
              <span className='text-sm font-light'>{t('from_price.from')}</span> ${tour?.cost.toFixed(2)}
            </h2>
            <hr />
            <div className='p-5'>
              <div className='flex justify-between'>
                <div>
                  <span className='text-sub-color-primary text-xs'>{t('from_price.duration')}</span>
                  <p className='font-semibold text-sm'>
                    {tour?.duration} {t('from_price.days')} - {tour?.duration && tour?.duration - 1}{' '}
                    {t('from_price.night')}
                  </p>
                </div>
                <div>
                  <span className='text-sub-color-primary text-xs'>{t('from_price.type')}</span>
