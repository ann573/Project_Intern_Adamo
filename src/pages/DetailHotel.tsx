...
const HotelDetail = () => {
  ...
  const { data: dataRelated, isLoading: isLoadingRelated } = useRelatedHotels(data?.typeroom as number)
  const [choose, setChoose] = useState(1)

  const totalRating =
    data &&
    (data.description.reviews.reduce((acc, cur) => acc + cur.rating, 0) / data.description.reviews.length).toFixed(2)

  if (isLoading) {
    return <DetailSkeleton />
  }

  const heading = [`${t('selectRoom')}`, `${t('description')}`, `${t('review')}`]
  ...
  return (
    <main className='max-w-[1200px] mx-auto xl:px-0 px-5 mb-20'>
      ...
      <section>
        <h1 className='md:text-4xl text-3xl font-semibold md:w-2/3'>{data?.name}</h1>
        ...
        <div className='flex gap-5 text-sub-color-primary mb-10 '>
          <div className='flex gap-2 items-center'>
            <p className='bg-[#FF7B42] text-white center py-1 px-2 font-semibold'>
              <span className='text-sm mr-1 font-light'>{t('rating')}</span> {totalRating}
            </p>
          ...
          <div className='order-1 lg:order-2 md:col-span-1 col-span-2'>
            <div className='bg-secondary dark:bg-secondary sticky top-5'>
              <h2 className='p-5 font-medium text-xl'>
                <span className='text-sm font-light'>{t('from')}</span> ${data?.cost.toFixed(2)}
              </h2>
              <hr />
              <div className='p-5'>
      ...
      <section className='my-20'>
        <h3 className='text-heading-second mt-5 text-3xl font-bold mb-5'>{t('related')}</h3>
        {isLoadingRelated ? (
          <CardSkeleton />
        ) : (
          <div className='grid md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-7'>
            {dataRelated?.map((item, index) => <CardHotel data={item} key={index} />)}
          </div>
        )}
      </section>
    </main>
