  ...
  return (
    <>
      <div>
        ...
        <p className='text-color-content-second'>{tour?.overview.description}</p>
        <ul className='my-10 list-inside'>
          {tour?.overview.highlight.map((item, index) => {
            return (
              <li key={index} className='list-disc text-color-content-second my-2'>
                {item}
              </li>
            )
          })}
      ...
      <div>
        ...
        <h4 className='font-bold mb-2 text-lg text-[#1e1e1e]'>{t('desc.departure.point')}</h4>
        ...
        <h4 className='font-bold mb-2 text-lg text-[#1e1e1e]'>{t('desc.departure.time')}</h4>
      ...
      <Accordion type='single' collapsible className='w-full'>
        {tour?.itinerary.map((item, index) => {
          return (
            <AccordionItem key={index} value={`item-${index}`} className=' transition-colors'>
              <AccordionTrigger className='text-[#04316A] dark:text-[#2f7bdf] font-semibold text-lg'>
                Day {index + 1}: {item.title} ({item.details.length} {item.details.length > 1 ? 'stops' : 'stop'})
              </AccordionTrigger>
              <AccordionContent>
                  ...
                  return (
                    <div key={index} className='flex gap-5 mb-10 first:mt-10'>
                      ...
                      <div className='text-color-content-second'>
                        <h3 className='text-[#04316A] dark:text-[#2f7bdf] font-semibold'> {detail.location}</h3>
                        <p className='mb-5 mt-2 leading-8'>{detail.description}</p>

                        {detail.duration && (
                          <div>
                            <p className='font-bold mb-1'>
                              Duration: <span className='font-light'>{detail.duration}</span>
                            </p>
                          </div>
                        )}
      ...
      <div className='relative cursor-pointer hover:opacity-90'>
        ...
        <img src={icon360} alt='icon_360' className='absolute inset-0 mx-auto my-auto' />
      </div>
