    ...
    <>
      <div className='py-5 container_custom mx-auto flex items-center'>
          ...
          <Link to='/'>
            <img
              src={
                ...
                ((pathname.includes('/hotel/') || pathname.includes('/policy') || pathname.includes('/tour/')) &&
                  theme === 'light')
                  ? logo_black
                  : logo
              }
                  ...
                  <li className='bg-orange text-white px-4 py-2 rounded-full cursor-pointer relative select-none '>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent className='w-56 mt-5'>
                        <DropdownMenuLabel>
                          {t('hello')} <span className='font-bold text-base'>{user.name}</span>
                        </DropdownMenuLabel>
                        <DropdownMenuSeparator />
                        <DropdownMenuGroup>
        ...
        <div className='md:hidden block'>
          <i className='ri-menu-line text-2xl cursor-pointer' onClick={() => setIsOpen(true)}></i>
        </div>
                ...
                <ul className='flex flex-col gap-5 text-lg text-black '>
                  <li>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent className='w-32 mt-2'>
                        <DropdownMenuItem className={`cursor-pointer`} onClick={() => handleLanguageChange('vi')}>
                          <span>Tiếng Việt</span>
                        </DropdownMenuItem>
                        <DropdownMenuItem className={`cursor-pointe`} onClick={() => handleLanguageChange('en')}>
                          <span>English</span>
                        </DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
