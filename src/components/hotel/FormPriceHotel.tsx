...
const FormPriceHotel = ({ id }: { id: string }) => {
  ...
  useEffect(() => {
    if (data) {
      setPrice(
        data?.cost + children * (data?.cost / 2) + totalRoom + totalAddon
      );
    }
  ...
  const handleAddonCheck = (idx: number) => {
    setAddons((prev) =>
      prev.map((addon, i) =>
        i === idx
          ? { ...addon, checked: !addon.checked, count: !addon.checked ? 1 : 0 }
          : addon
      )
    );
  };
      ...
      prev.map((addon, i) =>
        i === idx
          ? {
              ...addon,
              count:
                Math.max(0, addon.count + delta) > adults
                  ? adults
                  : Math.max(0, addon.count + delta),
