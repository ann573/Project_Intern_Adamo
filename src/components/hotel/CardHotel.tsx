...
const CardHotel = ({ data }: { data: IHotel }) => {
  ...
  useEffect(() => {
    const total = data.description.reviews.reduce((acc, cur) => (acc += cur.rating), 0)
    setTotalRating(Number((total / data.description.reviews.length).toFixed(2)))
  }, [totalRating, data])
  return (
    <div className={`relative`}>
      <Link to={`/hotel/${data.id}`}>
        <BlurImage src={data.description.thumbnail} className='w-full h-[250px]' alt='thumbnail_tours' />
      </Link>
      ...
      <div className='mt-3 flex flex-wrap sm:justify-between'>
        <div className='flex gap-2 items-center '>
          ...
          <span className='text-xs text-sub-color-primary'>({data.description.reviews.length} reviews)</span>
        </div>

        <p className='text-sm text-sub-color-primary ml-auto'>
          from <span className='text-heading text-xl font-medium'>${data.cost.toFixed(2)}</span> /night
        </p>
      </div>

      <div className=' w-fit text-white px-4 center gap-1 py-[2px] absolute top-6/12'>
        {[...Array(data.typeroom)].map((_, i) => (
          <svg key={i} className='size-5 text-yellow-400 cursor-pointer' fill='currentColor' viewBox='0 0 20 20'>
